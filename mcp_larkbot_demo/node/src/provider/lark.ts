 ...
 * @returns 飞书卡片JSON结构 / Lark card JSON structure
 */
...
export class LarkChatProvider implements ChatProvider {
   ...
   * @param chatId 飞书聊天ID / Lark chat ID
   ...
   * @param _app Express应用实例（未使用，飞书使用WebSocket连接）/ Express app instance (unused, Lark uses WebSocket)
...
export class LarkWebhookChatProvider extends LarkChatProvider {
  async registerMessageReceiveEvent(
    ...
    const dispatcher = this.getLarkEventDispatcher(onMessage);
    app.use(Lark.adaptExpress(dispatcher, { autoChallenge: true }));
  }
}
