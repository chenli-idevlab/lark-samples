...
export class LarkService {
  /** 飞书HTTP客户端实例 / Lark HTTP client instance */
  larkClient = new Client({ appId: config.lark.appId, appSecret: config.lark.appSecret });

  /** 飞书WebSocket客户端实例 / Lark WebSocket client instance */
  larkWsClient = new WSClient({ appId: config.lark.appId, appSecret: config.lark.appSecret });
   ...
   * @param refreshAccessToken 刷新令牌 / Refresh token
   * @returns Promise<TokenResponse> 新的令牌响应 / New token response
   */
  async refreshAccessToken(refreshAccessToken: string): Promise<TokenResponse> {
    const res = await fetch('https://open.feishu.cn/open-apis/authen/v2/oauth/token', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json; charset=utf-8' },
      body: JSON.stringify({
