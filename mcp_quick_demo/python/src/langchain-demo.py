...
def create_lark_mcp_client():
    ...
    Returns:
        ...
        MultiServerMCPClient: 用于飞书集成的配置好的 MCP 客户端实例
    """
    return MultiServerMCPClient(
        {
            "lark-mcp": {  # Server identifier | 服务器标识符
                ...
                "env": {
                    # Lark app credentials for API access | 用于 API 访问的飞书应用凭证
                    "APP_ID": os.getenv("APP_ID"),  # Lark application ID | 飞书应用 ID
                    "APP_SECRET": os.getenv(
                        "APP_SECRET"
                    ),  # Lark application secret | 飞书应用密钥
                },
            }
        }
    )
