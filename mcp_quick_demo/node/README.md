...
本项目展示了两种不同的 Node.js 实现来集成模型上下文协议 (MCP)。它演示了如何连接到 Lark MCP 服务器，并使用 AI SDK 和 LangChain 框架与飞书文档交互，为选择首选的 AI 集成方法提供了灵活性。
...
1. **Node.js Version Compatibility | Node.js 版本兼容性**

   ```
   Error: Cannot use import statement outside a module
   ```

   Solution: Ensure Node.js 18+ and package.json has "type": "module"
   解决方案：确保 Node.js 18+ 且 package.json 中有 "type": "module"

2. **Missing Dependencies | 缺失依赖**
...
3. **Environment Variables Not Set | 环境变量未设置**
...
4. **MCP Server Connection Failed | MCP 服务器连接失败**
   ```
